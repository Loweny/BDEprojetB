...
<div id=conteneur class="boite">
	<div id=contenu>
		...
		//	on récupère toutes les lignes 
			$resultat = $cnx->query("select * FROM covoiturage, membre WHERE covoiturage.numMembre = membre.numMembre ORDER BY dateDepot DESC;");
		...
		?>
			<table>
			<tr>
			<td><h2>Liste des covoiturages</h2></td>

			<td><a class="btn btn-primary" href="ajoutCovoit.php";>Ajouter un covoiturage</a></td>
			</tr>	
			</table>
			<table class="table table-striped">
			<thead><tr class="success">
			<?php if($_SESSION['privilege']=="admin") {
				echo "<td>Nom</td><td>Prenom</td><td>Classe</td>"; } ?>
				<td>Destination</td>
				<td>Date départ</td>
				<td>Heure départ</td>
			...
			while ($covoit) { ?>
				<tr>
				<?php if($_SESSION['privilege']=="admin") { ?>
				<td><?php echo utf8_encode($covoit->nom); ?> </td>
				<td><?php echo utf8_encode($covoit->prenom); ?> </td>
				<td><?php echo utf8_encode($covoit->classe); } ?> </td>
